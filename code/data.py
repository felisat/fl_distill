...
def get_mnist(path):
    ...
    train_data = torchvision.datasets.MNIST(
        root=path + "MNIST", train=True, download=True, transform=transforms
    )
    test_data = torchvision.datasets.MNIST(
        root=path + "MNIST", train=False, download=True, transform=transforms
    )
...
def get_cifar10(path):
    ...
    train_data = torchvision.datasets.CIFAR10(
        root=path + "CIFAR", train=True, download=True, transform=transforms
    )
    test_data = torchvision.datasets.CIFAR10(
        root=path + "CIFAR", train=False, download=True, transform=transforms
    )
...
def get_svhn(path):
    ...
    data = torchvision.datasets.SVHN(
        root=path + "SVHN", split="train", transform=transforms, download=True
    )
    return data
...
def get_loaders(
    ...
    subset_idcs = split_dirichlet(
        train_data.targets, n_clients, n_data, classes_per_client
    )
    client_data = [
        torch.utils.data.Subset(train_data, subset_idcs[i]) for i in range(n_clients)
    ]

    label_counts = [
        np.bincount(np.array(train_data.targets)[i], minlength=10) for i in subset_idcs
    ]
...
def split_image_data(labels, n_clients, n_data, classes_per_client):
    ...
    label_idcs = {
        l: np.argwhere(np.array(labels[:n_data]) == l).flatten().tolist()
        for l in range(n_labels)
    }

    if classes_per_client == 0:
        idcs = np.random.permutation(n_data // n_clients * n_clients).reshape(
            n_clients, -1
        )
    else:
...
def split_dirichlet(labels, n_clients, n_data, alpha, double_stochstic=True):
    ...
    class_idcs = [
        np.argwhere(np.array(labels) == y).flatten() for y in range(n_classes)
    ]
...
def print_split(idcs, labels):
    ...
    for i, idccs in enumerate(idcs):
        split = np.sum(
            np.array(labels)[idccs].reshape(1, -1)
