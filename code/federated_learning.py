...
def run_experiment(xp, xp_count, n_experiments):
  ...
  clients = [Client(model_fn, optimizer_fn, loader, client_dataset=(client_data[i] if hp["distill_phase"] == "clients" else None), aux_dataset=distill_data, distill_loader=distill_loader, **hp) for i, loader in enumerate(client_loaders)]
  ...
  for c_round in range(1, hp["communication_rounds"]+1):
    ...
    if hp["aggregation_mode"] in ["FD", "FAD", "FknnD"]:
      ...
      distill_stats = server.distill(participating_clients, hp["distill_epochs"], mode=hp["distill_mode"], distill_phase=hp["distill_phase"])
      xp.log({"distill_{}".format(key) : value for key, value in distill_stats.items() if key != "softlabels"})
