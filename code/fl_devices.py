...
class Device(object):
    ...
    def distill(self, clients=None, epochs=1, mode="regular", distill_phase="server"):
        ...
        if distill_phase == "server":
            ...
            epochs = (
                1  # for the one-step distillation only the softlabels are necessary
            )
        ...
        for ep in range(epochs):
            ...
            for x, _ in tqdm(self.distill_loader):
                ...
                if mode == "pate":
                    hist = torch.sum(
                        torch.stack([client.predict_(x) for client in clients]), dim=0
                    )
                ...
                if distill_phase == "server":
                    ...
                    def kulbach_leibler_divergence(predicted, target):
                        return (
                            -(target * torch.log(predicted.clamp_min(1e-7)))
                            .sum(dim=-1)
                            .mean()
                            - -1
                            * (
                                target.clamp(min=1e-7)
                                * torch.log(target.clamp(min=1e-7))
                            )
                            .sum(dim=-1)
                            .mean()
                        )
        ...
        if distill_phase == "clients":
            ...
            batch_size = self.kwargs["batch_size"] - int(
                self.kwargs["local_data_percentage"] * self.kwargs["batch_size"]
            )
            if batch_size > 0:
                distill_client_loader = DataLoader(
                    TensorDataset(self.aux_data, softlabels.to("cpu")),
            ...
            return {
                "loss": 1,
                "acc": [
                    eval_op(c.model, self.loaders)["accuracy"] for c in self.clients
                ],
                "epochs": 1,
            }
...
class Client(Device):
    ...
    def predict_(self, x):
        ...
        with torch.no_grad():
            ...
            amax = torch.argmax(
                y_, dim=1
            ).detach()  # (torch.cumsum(y_, dim=1)<torch.rand(size=(y_.shape[0],1))).sum(dim=1)
    ...
    def predict_knn(self, x, k=10, n_classes=10):
        ...
        with torch.no_grad():
            ...
            sim_labels = torch.gather(
                self.feature_labels.expand(x.size(0), -1).to(device),
                dim=-1,
                index=sim_indices.to(device),
            )
            # sim_weight = (sim_weight / temperature).exp()

            # counts for each class
            one_hot_label = torch.zeros(
                x.size(0) * k, n_classes, device=sim_labels.device
            )
            # [B*K, C]
            one_hot_label = one_hot_label.scatter(
                dim=-1, index=sim_labels.long().view(-1, 1), value=1.0
            )
            # weighted score ---> [B, C]
            pred_scores = torch.sum(
                one_hot_label.view(x.size(0), -1, n_classes)
...
def train_op(model, loaders, optimizer, scheduler, epochs, **kwargs):
    ...
    for ep in range(epochs):
        ...
        for x, y, source in loaders:
            ...
            loss = (
                local_loss
                + kwargs["distill_weight"]
                * warmup(
                    kwargs["c_round"], kwargs["max_c_round"], type=kwargs["warmup_type"]
                )
                * distill_loss
            )
            loss = loss / x.size(0)
