...
def run_experiment(xp, xp_count, n_experiments):
    ...
    client_loaders, test_loader, label_counts = data.get_loaders(
        train_data,
        test_data,
    ...
    clients = [
        Client(model_fn, optimizer_fn, loader, idnum=i)
    ...
    if hp["pretrained"]:
        for device in clients + [server]:
            device.model.load_state_dict(
                torch.load(args.CHECKPOINT_PATH + hp["pretrained"], map_location="cpu"),
                strict=False,
            )
    ...
    xp.log(
        {"server_val_{}".format(key): value for key, value in server.evaluate().items()}
    )
    for c_round in range(1, hp["communication_rounds"] + 1):

        participating_clients = server.select_clients(clients, hp["participation_rate"])
        xp.log(
            {"participating_clients": np.array([c.id for c in participating_clients])}
        )
        ...
        if hp["aggregation_mode"] in ["FD", "FAD", "FknnD"]:
            ...
            distill_stats = server.distill(
                clients, hp["distill_epochs"], mode=hp["distill_mode"]
            )
            xp.log(
                {
