...
def generate_plot(data, name):
    ...
    distill_acc = pd.DataFrame(
        data["distill_acc"], columns=[f"client{i}" for i in range(count)]
    )
    distill_acc.index.name = "round"
    distill_acc = distill_acc.reset_index().melt("round")

    base = (
        alt.Chart(distill_acc)
        .transform_calculate(
            line="'Average'",
    ...
    plot = (mean + band).resolve_scale(color="independent")
    return plot


parser = argparse.ArgumentParser(description="Generate plots from experiments")
parser.add_argument(
    "sourcedir", default="./", help="Path to where the experiments are stored"
)
parser.add_argument(
    "targetdir", default="./", help="Path to where the plots should be stored"
)
